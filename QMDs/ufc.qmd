...
Now we can fit a simple model first so I would like to start with a logistic regression model in which we first have to transform our variable that we are trying to predict (Winner) into 0s and 1s

```{python}
ufc['target'] = ufc['Winner'].map({'Red': 0, 'Blue': 1})
```
