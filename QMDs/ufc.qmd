...
The code above is an implementation of using the melt function for the purpose stated above if we had to, instead we can just drop the 24 variables as shown in the following code chunk. Now we have 97 variables. 
...
As we can see the majority of the data is Non-Championship data so for that reason I want to create two datasets as stated prior one that has all observations and the other only championship bouts. Lets take a look at the distribution of wins regarding only championship bouts.
...
sns.countplot(
    data=ufc_clean_champ_only, 
    x='Winner', 
    palette={'Red': 'crimson', 'Blue': 'royalblue'},
    order=['Red', 'Blue']
)
plt.title('Win Distribution in UFC Championship Bouts', fontsize=15)
plt.xlabel('Corner Color')
plt.ylabel('Number of Wins')
plt.show()
```

Now lets take a look at the prior distribution regarding the whole data set
```{python}
sns.set_theme(style="whitegrid")
plt.figure(figsize=(8, 6))
sns.countplot(
    data=ufc_clean_total, 
    x='Winner', 
    palette={'Red': 'crimson', 'Blue': 'royalblue'},
    order=['Red', 'Blue']
)
plt.title('Win Distribution in UFC Bouts', fontsize=15)
plt.xlabel('Corner Color')
plt.ylabel('Number of Wins')
plt.show()
```

As we can see in both circumstances the Red corner is more likely to win and has a slightly higher chance to win in a championship bout over a non-championship bout.
...
#here we test the model on the test set (unseen data) hopefully we get a good result
